...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-white text-slate-900 antialiased min-h-screen flex flex-col">
        ...
        <!-- ==================== NAVBAR ==================== -->
        <header class="w-full border-b border-slate-100 bg-white/85 backdrop-blur-md sticky top-0 z-50">
            <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-slate-900 font-extrabold text-xl tracking-tight">CampusReserve</span>
                </div>
                    ...
                    <nav class="flex items-center gap-6">
                        @auth
                            <a href="{{ route('dashboard') }}"
                               class="px-5 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors shadow-sm">
                                Dashboard
                            </a>
                        @else
                            <a href="{{ route('login') }}"
        ...
        <main class="flex-1 flex flex-col justify-center py-16 md:py-24">
            <div class="max-w-5xl mx-auto px-6 text-center">
                ...
                {{-- Centered Typography --}}
                <h1 class="text-4xl sm:text-5xl md:text-6xl font-black text-slate-950 tracking-tight leading-none mb-6">
                    Find your perfect study environment.
                </h1>
                
                <p class="text-lg md:text-xl text-slate-600 max-w-3xl mx-auto mb-10 leading-relaxed font-medium">
                    Stop wandering the halls. Reserve soundproof pods, collaboration tables, and quiet desks instantly.
                </p>

                {{-- CTAs Stack (Matches Screenshot Proportionally) --}}
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4 max-w-md mx-auto mb-16">
                    @auth
                        <a href="{{ route('dashboard') }}"
                           class="w-full sm:flex-1 py-3.5 px-6 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow-md transition-all text-center tracking-wide text-sm uppercase">
                            Go to App Grid
                        </a>
                    @else
                        <a href="{{ route('login') }}"
                        ...
                        <a href="{{ route('register') }}"
                           class="w-full sm:flex-1 py-3.5 px-6 bg-white hover:bg-slate-50 text-slate-800 font-bold border border-slate-350 rounded-lg shadow-sm transition-all text-center tracking-wide text-sm uppercase">
                            Learn More
                        </a>
                    @endauth
                </div>
                ...
                <div class="max-w-4xl mx-auto rounded-3xl overflow-hidden shadow-2xl border border-slate-100 relative group">
        ...
        <!-- ==================== FOOTER ==================== -->
        <footer class="py-8 border-t border-slate-100 bg-slate-50/50">
